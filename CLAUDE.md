...
- **`ServerManager`** is the core controller:
  ...
  - `handleLogic()` (called every `loop()` iteration) evaluates each device's mode against the current time via `isInside()` and drives the GPIO pins. **Safety interlock**: the pH/chlorine pumps only run in `AUTO`/`ON` mode if the ESP32 is still driving the pool pump's own output pin HIGH (`flowOk = digitalRead(_pool.pin)`) — this prevents dosing whenever the firmware has turned the pool pump off, but it reads back the ESP32's own GPIO register, not an actual flow sensor or relay feedback signal, so a relay/motor failure that leaves the pin driven HIGH would not be caught.
  ...
  - Web routes are registered in `begin()`: `GET /` returns a self-contained HTML/CSS/JS dashboard (built as inline C++ string concatenation), `POST /set?dev=<pool|ph|cl>&m=<0|1|2>` changes a device's mode, `GET /status` returns JSON state that the dashboard polls every second. All three routes go through `requireAuth()`, which enforces HTTP Basic Auth (`Config::WEB_AUTH_USER`/`WEB_AUTH_PASSWORD`) only if `Config::WEB_AUTH_ENABLED` is `true` (default `false`). `/set` additionally goes through `requireSameOrigin()`, which rejects requests carrying an `Origin` header that doesn't match the device's own `Host` (basic CSRF defense; requests with no `Origin` header, e.g. from curl/scripts, are let through).
