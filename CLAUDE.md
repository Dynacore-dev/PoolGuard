...
- **`ServerManager`** is the core controller:
  ...
  - Web routes are registered in `begin()`: `GET /` returns a self-contained HTML/CSS/JS dashboard (built as inline C++ string concatenation), `GET /set?dev=<pool|ph|cl>&m=<0|1|2>` changes a device's mode, `GET /status` returns JSON state that the dashboard polls every second. All three routes go through `requireAuth()`, which enforces HTTP Basic Auth (`Config::WEB_AUTH_USER`/`WEB_AUTH_PASSWORD`) only if `Config::WEB_AUTH_ENABLED` is `true` (default `false`, since `/set` has no other safeguard against remote/CSRF abuse if the network isn't trusted).
